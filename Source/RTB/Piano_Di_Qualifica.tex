...
\begin{table}[h!]
    ...
    \begin{tabular}{c p{7cm} c c}
        ...
        MPC-13 & \textbf{Quality metrics satisfied} & $\geq 80\%$ & $100\%$ \\
    \end{tabular}
...
\begin{table}[h!]
    ...
    \begin{tabular}{c p{7cm} c c}
        ...
        MPC-14 & \textbf{Time Efficiency} & $\geq 60\%$ & $100\%$ \\
        MPC-15 & \textbf{Sprint Goal Achievement} & $\geq 80\%$ & $100\%$ \\
    \end{tabular}
...
\begin{table}[h!]
    ...
    \begin{tabular}{c p{7cm} c c}
        \rowcolor{AccentColor}
        \textcolor{white}{\textbf{ID}} & \textcolor{white}{\textbf{Nome}} & \textcolor{white}{\textbf{Accettabile}} & \textcolor{white}{\textbf{Ottimo}} \\
        MPD-05 & \textbf{Code Coverage} & $\geq 70\%$ & $\geq 80\%$ \\
...
\begin{table}[h!]
    ...
    \begin{tabular}{c p{7cm} c c}
        \rowcolor{AccentColor}
        \textcolor{white}{\textbf{ID}} & \textcolor{white}{\textbf{Nome}} & \textcolor{white}{\textbf{Accettabile}} & \textcolor{white}{\textbf{Ottimo}} \\
        MPD-10 & \textbf{Facilità di utilizzo} & $\leq 7$ click & $\leq 5$ click\\
        MPD-11 & \textbf{Tempo medio di apprendimento} & $\leq 5$ minuti & $\leq 2$ minuti\\
    \end{tabular}
...
Di seguito sono riportati i test di sistema definiti per verificare la copertura dei requisiti funzionali, derivati direttamente dai casi d'uso descritti nel documento di Analisi dei Requisiti.

\begin{center}
\rowcolors{2}{LightGray}{white}
\renewcommand{\arraystretch}{1.5} % Spaziatura righe come nel tuo esempio
\begin{longtable}{|>{\centering\arraybackslash}p{2cm}|>{\raggedright\arraybackslash}p{9cm}|>{\centering\arraybackslash}p{2.5cm}|>{\centering\arraybackslash}p{1.5cm}|}
\hline
\rowcolor{AccentColor}
\textcolor{white}{\textbf{ID Test}} & \textcolor{white}{\textbf{Descrizione}} & \textcolor{white}{\textbf{Requisito}} & \textcolor{white}{\textbf{Stato}} \\
\hline
\endhead

% --- GESTIONE UTENTE ---
TS-01 & Verificare che un utente non autenticato possa effettuare il Login inserendo email e password corrette. (UC1) & R-1-F & NI \\
\hline
TS-02 & Verificare che il sistema restituisca un errore in caso di Login con credenziali errate. (UC1) & R-1-F & NI \\
\hline
TS-03 & Verificare che un nuovo utente possa completare la procedura di Registrazione inserendo i dati richiesti. (UC2) & R-2-F & NI \\
\hline
TS-04 & Verificare che un utente autenticato possa effettuare il Logout e venga reindirizzato alla pagina di benvenuto. (UC3) & R-3-F & NI \\
\hline
TS-05 & Verificare che un utente autenticato possa visualizzare i propri dati nella pagina Profilo. (UC4) & R-4-F & NI \\
\hline
TS-06 & Verificare che un utente possa modificare la propria password dalla sezione di gestione profilo. (UC5) & R-5-F & NI \\
\hline

% --- GESTIONE REPOSITORY ---
TS-07 & Verificare che l'utente possa inserire l'URL di un nuovo repository GitHub per avviarne l'analisi. (UC6) & R-6-F & NI \\
\hline
...
In questa sezione vengono esposte le misurazioni effettuate durante il periodo corrente. L'analisi dei grafici permette di valutare l'andamento del progetto rispetto alle previsioni.

\subsection{Earned Value e Planned Value (MPC-01/02)}
Il confronto tra EV e PV permette di capire se il progetto è in linea con i tempi previsti.
\begin{figure}[h!]
    \centering
    ...
    \fbox{\begin{minipage}{0.8\textwidth}
        \centering
        \vspace{4cm}
        \textit{Grafico dell'andamento EV vs PV (Dati non ancora disponibili per questo sprint)}
        \vspace{4cm}
    \end{minipage}}
    \caption{Andamento di Earned Value e Planned Value}
\end{figure}

\subsection{Requirements Stability Index (MPC-08)}
Questo indice monitora la stabilità dei requisiti nel tempo. Variazioni significative indicano scoperte tardive di nuovi requisiti.
\begin{figure}[h!]
    \centering
    % Placeholder per il grafico
    \fbox{\begin{minipage}{0.8\textwidth}
        \centering
        \vspace{3cm}
        \textit{Grafico dell'andamento RSI}
        \vspace{3cm}
    \end{minipage}}
    \caption{Andamento della stabilità dei requisiti}
\end{figure}

\subsection{Indice di Gulpease (MPC-09)}
Monitoraggio della leggibilità della documentazione prodotta (Manuale Utente, Analisi dei Requisiti, ecc.).
\begin{figure}[h!]
    \centering
    % Placeholder per il grafico
    \fbox{\begin{minipage}{0.8\textwidth}
        \centering
        \vspace{3cm}
        \textit{Grafico dell'andamento dell'Indice di Gulpease per documento}
        \vspace{3cm}
    \end{minipage}}
