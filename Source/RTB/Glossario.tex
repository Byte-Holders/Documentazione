...
\begin{titlepage}
    ...
    \begin{tabularx}{0.9\textwidth}{@{}>{\bfseries\sffamily}l X@{}}
    	Autore & \sffamily Alessandro Morabito, Giulia Romanato\\
    	\arrayrulecolor{MediumGray}\hline \\[-1.5ex]
    	Verificatore & \sffamily Alessandro Morabito, Giulia Romanato \\
    	\arrayrulecolor{MediumGray}\hline \\[-1.5ex] 
...
\term{CVSS}
\textit{Common Vulnerability Scoring System}. Una metrica che associa un valore numerico compreso tra 0 (bassa gravità) e 10 (alta gravità) a una determinata vulnerabilità. A un valore numerico è associata anche una stringa (\textit{CVSS vector string}) che specifica i valori utilizzati per ottenerlo. Attualmente in versione \texttt{4.0}\footnote{Specifica CVSSv4.0 \lastVisited{17}{03}{2026}: \url{https://www.first.org/cvss/v4.0/specification-document}}.
...
\term{Dockerfile}
File contenente le istruzioni per la costruzione di un'\refterm{Immagine (Docker)}. A seguire istruzioni comuni\footnote{Documentazione Dockerfile \lastVisited{17}{03}{2026}: \url{https://docs.docker.com/reference/dockerfile}}:
\begin{itemize}
...
\begin{itemize}
    \item \texttt{git clone <url>}: clona un repository remoto in locale.
    \item \texttt{git add <file>}: aggiunge file specifici all'area di staging.
    \item \texttt{git add .}: aggiunge tutti i file modificati all'area di staging.
    \item \texttt{git rm <file>}: rimuove file specifici dal repository e dall'area di staging
    \item \texttt{git commit -m "messaggio"}: crea un commit con i file nell'area di staging e un messaggio descrittivo.
    \item \texttt{git push origin <branch>}: invia i commit locali al repository remoto sul branch specificato.
    \item \texttt{git pull origin <branch>}: recupera e integra le modifiche dal repository remoto al branch locale.
    \item \texttt{git status}: mostra lo stato dei file nel repository, inclusi quelli modificati, aggiunti o non tracciati.
    \item \texttt{git diff}: mostra le differenze tra i file modificati e l'ultima versione committata.
    \item \texttt{git branch}: elenca, crea o elimina branch nel repository.
    \item \texttt{git checkout <branch>}: passa a un branch specifico.
    \item \texttt{git switch <branch>}: alternativa a git checkout per cambiare branch.
    \item \texttt{git fetch}: recupera gli aggiornamenti dal repository remoto senza integrarli automaticamente.
    \item \texttt{git merge <branch>}: unisce un branch specifico nel branch corrente.
    \item \texttt{git rebase <branch>}: integra le modifiche di un branch specifico riscrivendo la cronologia dei commit.
\end{itemize}

\term{GitHub}
GitHub è una piattaforma di hosting per lo sviluppo software e il versionamento basato su \refterm{Git} che permette di:
\begin{itemize}
    \item collaborare allo sviluppo di software in modo distribuito,
    \item gestire issue, pull request, documentazione
    \item automatizzare processi tramite \refterm{GitHub Actions}
\end{itemize}
È ampiamente utilizzata per progetti open-source e collaborativi.

\term{Gitflow}
Gitflow è un workflow per \refterm{Git} basato su branch multipli con ruoli specifici. Permette una chiara separazione tra sviluppo, rilascio e manutenzione.\\
La struttura principale include:
\begin{itemize}
    \item \texttt{master}: contiene solo versioni pronte al rilascio
    \item \texttt{develop}: rappresenta lo stato corrente dello sviluppo
    \item \texttt{feature branches}: uno per ogni nuova funzionalità
    \item \texttt{release branches}: preparazione della versione
    \item \texttt{hotfix branches}: correzioni urgenti sul master
\end{itemize}
I principali comandi di gitflow sono:
\begin{itemize}
  \item \texttt{git flow init}: inizializza un repository esistente in modo che possa usare Gitflow
  \item \texttt{git flow feature start <nome-feature>}: crea e passa a un nuovo branch di funzionalità.
  \item \texttt{git flow feature finish <nome-feature>}: unisce il branch di funzionalità nel branch develop e lo elimina.
  \item \texttt{git flow feature publish <nome-feature>}: pubblica il branch di funzionalità sul repository remoto.
  \item \texttt{git flow feature pull origin <nome-feature>}: recupera e integra le modifiche dal branch di funzionalità remoto.
  \item \texttt{git flow feature track <nome-feature>}: crea un branch di funzionalità locale che traccia il branch remoto.
  \item \texttt{git flow release start release [BASE] <versione>}: crea un nuovo branch di release a partire dal branch specificato (di default develop).
  \item \texttt{git flow feature release publish <versione>}: pubblica il branch di release sul repository remoto.
  \item \texttt{git flow release finish <versione>}: unisce il branch di release nei branch master e develop, crea un tag per la versione e elimina il branch di release.
  \item \texttt{git flow hotfix start <nome-hotfix> [BASE]}: crea un nuovo branch di hotfix a partire dal branch specificato (di default master).
  \item \texttt{git flow hotfix finish <nome-hotfix>}: unisce il branch di hotfix nei branch master e develop, crea un tag per la versione e elimina il branch di hotfix.
\end{itemize}
...
\GlossarioLettera{L}
\term{Langchain}
\refterm{Framework} open source per lo sviluppo di applicazioni basate su \refterm{LLM}. Consente l'esecuzioni di workflow semplici.

\term{Langgraph}
\refterm{Framework} open source per lo sviluppo di applicazione basate su \refterm{LLM}. Di basso livello, è concentrato principalmente sull'orchestrazione di agenti. I workflow sono compilati a partire da un grafo, personalizzabile.

\term{Large Language Model}
Tipologia di modello di \textit{machine learning} addestrato per \textit{Natural Language Processing}, particolarmente per la generazione di testo. Alla base dei chatbot.
...
\term{MongoDB}
Un \refterm{Database non relazionale} orientato ai documenti BSON (estensione del \refterm{JSON}, salva dati in binario). Permette anche la creazione di \refterm{Database vettoriale}.
...
\begin{itemize}
	...
	\item \textbf{Punteggio di rischio\footnote{Formula per il calcolo del punteggio del rischio OWASP \lastVisited{17}{03}{2026}: \url{https://owasp.org/Top10/2025/0x02_2025-What_are_Application_Security_Risks/}}}: calcolato utilizzando i dati soprastanti. Definisce la posizione nel ranking della Top 10
\end{itemize}

\term{Owner}
Ruolo che fa riferimento al creatore di uno workspace. Gode degli stessi permessi di un \refterm{Project Manager} e non può essere rimosso dagli workspace che ha creato.
...
\begin{itemize}
  ...
  \item \textbf{Pianificazione e Organizzazione}: definisce il piano di progetto, stabilisce scadenze, milestone e priorità operative
  ...
  \item \textbf{Assegnazione delle Risorse/Assegnazioni (chi-fa-cosa)}: Decide a quali \refterm{Developer} assegnare i vari progetti di cui è responsabile
...
\begin{itemize}
	...
	\item \refterm{Syft}
	\item \refterm{GitHub} \refterm{REST} \refterm{API}
\end{itemize}
...
\term{Report}
Prodotto finale di un \refterm{Sistema ad agenti}. Comprende sezioni per:
\begin{itemize}
	\item analisi statica del codice (\refterm{semgrep})
	\item \refterm{Code coverage}
	\item analisi della documentazione (utilizzando un \refterm{LLM} dedicato)
	\item informazioni generali sul repo (\refterm{GitHub} \refterm{REST} \refterm{API})
	\item dipendenze (\refterm{Syft})
\end{itemize}

\term{Requisito}
Una condizione che un sistema o componente deve rispettare, soddisfacendo una specifica definita nell'\refterm{Analisi dei Requisiti}.
...
\term{Tech Lead}
Un tech lead (o technical lead) è il supervisore tecnico di più progetti. È la figura che unisce visione tecnica e capacità di coordinamento, fungendo da ponte tra il \refterm{Project Manager} e il team di sviluppo.\\
...
\term{Tool}
Un tool è uno strumento esterno che estende le capacità di un \refterm{Agente}, permettendo ad esempio l'esecuzione di codice e recuperare informazioni aggiornate.
