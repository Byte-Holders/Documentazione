...
\begin{titlepage}
    ...
    \vspace{0.5cm}
    {\large \color{AccentColor}\sffamily 01 Aprile 2026}\par
...
\begin{enumerate}
    \item Discussione sullo stato della progettazione
...
\subsection{Discussione sullo stato della progettazione}
Il gruppo ha discusso lo stato attuale della progettazione, in relazione all'incontro con il prof. Cardin. I problemi principali riscontrati rigurdano la correttezza dell'implementazione dell'architettura layered del backend, si è discusso quindi di:
\begin{itemize}
    \item introdurre interfacce tra i layer per ridurre l'accoppiamento tra i layer
    \item suddividere i service in lassi con meno responsabilità, in modo da semplificare la gestione delle dipendenze
\end{itemize}
Per quando riguarda la progettazione del frontend, è confermata in via definitava ls scelta di adottare il model-view-viewmodel (MVVM) come pattern architetturale.

\subsection{Retrospettiva dello sprint}
La continuazione della progettazione si è svolta con alti e bassi, alternando decisioni che ci hanno soddisfatto a momenti di difficoltà nel trovare fonti utili per risolvere i dubbi.\\ L'incontro con il prof. Cardin ci ha permesso di individuare altri problemi e ci ha costretto a ripensare parti della progettazione. Ad ogni modo con le ultime decisioni prese dovrebbe essere quasi giunta al termine. \\Anche il setup iniziale dello sviluppo ha dovuto rallentare a causa dei problemi sopracitati.

\subsection{Definizione degli obiettivi del prossimo sprint}
Data l'imminente conclusione della fase di progettazione, il gruppo ha quindi deciso di cominciare la fase di sviluppo, suddividendo lo sforzo necessario in task assegnate ai singoli membri del gruppo. 
...
\begin{center}
    ...
    \begin{tabular}{>{\centering\arraybackslash}m{3cm} >{\raggedright\arraybackslash}p{6cm} >{\centering\arraybackslash}p{3.5cm}}
        ...
        DEC-PB-016 & scelta pattern architetturale frontend model-view-viewmodel (MVVM) & tutto il gruppo \\
        DEC-PB-017 & introduzione interfacce tra i layer del backend e suddivisione dei service & tutto il gruppo \\ 
        \midrule
        AZ-PB-016 & continuazione progettazione & Alessandro Morabito, Giulia Romanato, Nicolò Lattanzio, Lorenzo Grolla \\ 
        AZ-PB-017 & sviluppo task relativa a gestione workspace & Giulia Romanato \\
        AZ-PB-018 & sviluppo task relativa a gestione user e guardia per token JWT & Damiano Berti \\
        AZ-PB-019 & sviluppo task relativa a gestione repository nel workspace & Lorenzo Grolla \\
        AZ-PB-020 & sviluppo task relativa alla gestione degli  scan & Nicolò Lattanzio \\
        AZ-PB-021 & sviluppo task relativa agli inviti & Giacomo Nalotto \\
    \end{tabular}
\end{center}
\end{document}